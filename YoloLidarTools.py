...
def dsiplay_metrics(train_dir):
  sns.set_style('darkgrid')
  # read in the results.csv file as a pandas dataframe
  df = pd.read_csv('./runs/detect/'+train_dir+'/results.csv')
  ...
  img = matplotlib.image.imread('./runs/detect/'+train_dir+'/confusion_matrix.png')
...
def plot_bboxes(image, boxes, labels=[], colors=[], score=True, conf=None):
  #Define COCO Labels
  if labels == []:
    labels = {0: u'__background__', 1: u'building'}
...
def convert_bbox(file, zip=True):
  names = []
  file = Path(file)
